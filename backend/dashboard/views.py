    ...
    def get(self, request):
        try:
            ...
            overdue_payments = Membership.objects.filter(
                payment_status="overdue", status="active"
            ...
            return Response(
                {
                    ...
                    "unreadCount": len(
                        [
                            ...
                            if n["priority"] in ["high", "medium"]
                        ]
                    ),
                    "categories": {
                        "high": len(
                            [n for n in notifications if n["priority"] == "high"]
                        ),
                        "medium": len(
                            [n for n in notifications if n["priority"] == "medium"]
                        ),
                        "low": len(
                            [n for n in notifications if n["priority"] == "low"]
                        ),
                    },
            ...
            return Response(
                {"error": "Could not retrieve notifications", "details": str(e)},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
