...
class AnalyticsService:
    ...
    def _get_overview_analytics(date_range):
        ...
        new_members_current = Member.objects.filter(
            registration_date__gte=date_range['start']
        ).count()

        previous_start = date_range['start'] - (date_range['end'] - date_range['start'])
        new_members_previous = Member.objects.filter(
            registration_date__gte=previous_start,
            registration_date__lt=date_range['start']
        ).count()
